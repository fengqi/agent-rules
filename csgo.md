...
- 遵守 `controller/`、`service/`、`remote/`、`model/`、`dao/`、`conf/` 分层：`controller` 只做参数、权限、调用和响应，业务放 `service`，数据库操作放 `dao`，外部接口放 `remote`，可能复用的结构体放 `model`，尽量不在 `model` 以外的地方定义struct结构体，除非临时使用等特殊情况。
...
- Redis、MySQL、HTTP/RPC、Kafka 分别使用 `csredis`、`csmysql`、`cshttp`、`cskafka` 或项目封装，相关 namespace、实例名、服务名必须在 dev、pre、pro 配置中声明。
